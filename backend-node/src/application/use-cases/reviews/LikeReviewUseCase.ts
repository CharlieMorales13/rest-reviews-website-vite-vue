...
interface LikeReviewDTO { userId: string; reviewId: string; }
...
export class LikeReviewUseCase {
  ...
  async execute(dto: LikeReviewDTO): Promise<{ likesCount: number; likedByMe: true }> {
    ...
    if (review.userId === dto.userId) throw new AppError("No puedes dar like a tu propia reseña", 403);
    ...
    this.createNotificationUseCase.execute({ userId: review.userId, reviewId: dto.reviewId, type: "like" }).catch(() => {});
