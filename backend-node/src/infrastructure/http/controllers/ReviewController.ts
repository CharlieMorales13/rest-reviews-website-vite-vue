...
export class ReviewController {
  constructor(
    ...
    @inject(ListEstablishmentReviewsUseCase)
    private listEstablishmentReviewsUseCase: ListEstablishmentReviewsUseCase,
    ...
    @inject(ReplyToReviewUseCase)
    private replyToReviewUseCase: ReplyToReviewUseCase,
    @inject(UpdateReviewUseCase)
  ...
  public getAll = async (req: Request, res: Response): Promise<void> => {
    ...
    const { data, total } = await this.listReviewsUseCase.execute({
      page: pageNum,
      limit: limitNum,
    });
  ...
  public reply = async (
    ...
    const review = await this.replyToReviewUseCase.execute(
      validatedData,
      user.role,
    );
