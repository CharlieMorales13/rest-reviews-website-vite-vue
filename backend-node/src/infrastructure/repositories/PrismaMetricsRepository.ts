...
export class PrismaMetricsRepository implements IMetricsRepository {
  async getGlobalSummary(): Promise<GlobalMetrics> {
    ...
    const topEstablishments = rawEstablishments
      .map((e: any) => {
        ...
        const positiveReviews = e.reviews.filter(
          (r: any) => r.sentimentResults[0]?.predictedLabel === "positive",
        ).length;
        ...
        return {
          ...
          sentimentScore:
            e._count.reviews > 0
              ? (positiveReviews / e._count.reviews) * 100
              : 0,
        };
      })
    ...
    return {
      sentimentDistribution: {
        positive:
          sentimentCounts.find((s: any) => s.predictedLabel === "positive")
            ?._count._all ?? 0,
        neutral:
          sentimentCounts.find((s: any) => s.predictedLabel === "neutral")
            ?._count._all ?? 0,
        negative:
          sentimentCounts.find((s: any) => s.predictedLabel === "negative")
            ?._count._all ?? 0,
        total: reviewCount,
      },
      topEstablishments,
  ...
  async getEstablishmentSummary(
    id: string,
  ): Promise<EstablishmentMetricSummary | null> {
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const endOfLastMonth = startOfMonth;

    const [e, reviewsThisMonth, reviewsLastMonth, latestSnapshot] =
      await Promise.all([
    ...
    const sentimentCounts = {
      positive: e.reviews.filter(
        (r: any) => r.sentimentResults[0]?.predictedLabel === "positive",
      ).length,
      neutral: e.reviews.filter(
        (r: any) => r.sentimentResults[0]?.predictedLabel === "neutral",
      ).length,
      negative: e.reviews.filter(
        (r: any) => r.sentimentResults[0]?.predictedLabel === "negative",
      ).length,
    ...
    const positiveRatio =
      e._count.reviews > 0
        ? (sentimentCounts.positive / e._count.reviews) * 100
        : 0;
    ...
    const promoters = e.reviews.filter(
      (r: any) => (r.foodScore + r.serviceScore + r.priceScore) / 3 >= 4,
    ).length;
    const detractors = e.reviews.filter(
      (r: any) => (r.foodScore + r.serviceScore + r.priceScore) / 3 <= 2.5,
    ).length;
    const nps =
      e._count.reviews > 0
    ...
    const CRITICAL_WORDS = [
      // Comida cruda / mal cocida
      "crudo", "cruda", "crudos", "crudas",
      // Comida en mal estado
      "podrido", "podrida", "podridos", "podridas",
      "caducado", "caducada", "caducados",
      "vencido", "vencida", "vencidos",
      ...
      "intoxicado", "intoxicada", "intoxicación", "intoxicacion",
      "enfermé", "enfermamos", "enfermaron",
      "vómito", "vomito", "vomité", "vomite", "vomitar", "vomitando",
      "diarrea",
      "náuseas", "nauseas",
    ...
    const allCritical = e.reviews.filter(
      (r: any) =>
        r.comment &&
        CRITICAL_WORDS.some((w: string) =>
          (r.comment as string).toLowerCase().includes(w),
        ),
    );
    ...
    const negativeTerms: NegativeTerm[] = Array.isArray(
      latestSnapshot?.negativeTerms,
    )
