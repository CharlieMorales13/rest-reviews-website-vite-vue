    ...
    <div class="relative z-10 w-full max-w-md p-6 animate-fade-in">
      <div class="bg-white/5 backdrop-blur-2xl border border-white/20 rounded-[32px] p-8 md:p-12 shadow-2xl">
        ...
        <div class="text-center mb-10">
          ...
          <h1 class="text-3xl font-bold tracking-tight text-white mb-2">Nueva contraseña</h1>
          <p class="text-white/60 text-sm">Elige una contraseña segura para tu cuenta</p>
        </div>
        ...
        <div v-if="tokenMissing" class="text-center space-y-4">
          ...
          <router-link to="/forgot-password" class="block text-anahuac-orange text-sm font-medium hover:underline">
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
          <div class="space-y-2">
            <label class="block text-xs font-medium text-white/50 ml-1 uppercase tracking-wider">Nueva contraseña</label>
          ...
          <div class="space-y-2">
            <label class="block text-xs font-medium text-white/50 ml-1 uppercase tracking-wider">Confirmar contraseña</label>
            <div class="relative group">
              <span class="absolute left-4 top-1/2 -translate-y-1/2 text-lg opacity-40 group-focus-within:opacity-100 transition-opacity">🔒</span>
              <input
                :type="showPassword ? 'text' : 'password'"
          ...
          <div v-if="error" class="p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-sm">
            {{ error }}
            <router-link v-if="error.includes('expirado')" to="/forgot-password" class="block mt-2 text-anahuac-orange hover:underline">
              Solicitar nuevo enlace
            </router-link>
          </div>
